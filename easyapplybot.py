...
class EasyApplyBot:
    ...
    def applications_loop(self, position, location):
        ...
        log.info("2...Looking for jobs.. Please wait..")
        ...
        while time.time() - start_time < self.MAX_SEARCH_TIME:
            try:
                ...
                IDs: list = []
                log.debug("start looking...")
                # children selector is the container of the job cards on the left
                for link in links:
                    children = link.find_elements("xpath",
                        '//ul[@class="scaffold-layout__list-container"]'
                    )
                    children2 = link.find_elements(By.CLASS_NAME, 'job-card-container__metadata-item')
                    caution = False
                    for child in children2:
                        log.debug(f"job text: {child.text}")
            ...
            if is_present(error_locator):
                for element in self.browser.find_elements(error_locator[0],
                    ...
                    if "Please enter a valid answer" in text:
                        ...
                        follow_buttons = self.browser.find_element(By.XPATH, 
    ...
    def load_page(self, sleep=1):
        scroll_page = 1
        log.debug("load page....");
        while scroll_page < 1000:
            log.debug("scrolling...")
            self.browser.execute_script("window.scrollTo(0," + str(scroll_page) + " );")
            scroll_page += 200
            log.debug("scroll is over")
            time.sleep(sleep)
